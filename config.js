...
/* Era pra responde qnd mandar peppy aff

	var pes = (type === 'conversation' && mek.message.convesation) ? mek.message.conversation : (type == 'imageMassage') && mek message
	var pes = (type === 'conversation' && mek.message.conversation) ? mek.message.conversation : (type == 'imageMessage') && mek.message.imageMessage.caption ? mek.message.imageMessage.caption : (type == 'videoMessage') && mek.message.videoMessage.caption ? mek.message.videoMessage.caption : (type == 'extendedTextMessage') && mek.message.extendedTextMessage.text ? mek.message.extendedTextMessage.text : ''
	const messagesC = pes.slice(0).trim().split(/ +/).shift().toLowerCase()


		if (messagesC.includes("peppy")){
			client.updatePresence(from, Presence.composing)
			reply("fala meu consagrado")
		}
*/
		...
		switch(command) {
			...
			case 'about':
				await kill.sendFileFromUrl(from, 'https://i.imgur.com/kj6B10Z.png', 'iris.png', mess.about(), id);await kill.reply(from, mess.everhost(), id)
				break
			...
			case 'maconheiro':
				if (!isGroupMsg) return await kill.reply(from, mess.sogrupo(), id)
				await kill.reply(from, mess.wait(), id)
				await kill.sendTextWithMentions(from, mess.maco(randomMember))
				break
			...
			case 'ofc':
				if (isGroupMsg) return await kill.reply(from, mess.sopv(), id)
				await kill.sendLinkWithAutoPreview(from, 'https://chat.whatsapp.com/DfACwckzJsr6tV0ew3eMs8', '\nEsse grupo é apenas um grupo de zueira, não leve nada aqui para o pessoal ou a sério')
				break
		   ...
		   case 'translate':
			...
			case 'criador':
				await kill.reply(from, `☀️ - Host: Eu com meu pc da xuxa :)\n⭐ - Dev: Guilherme [https://wa.me/${config.Owner[0].replace('@c.us', '')}]\n🌙 - Insta - @Gui.dev [https://www.instagram.com/gui.dev/]`, id)
			...
			// Se quiser adicione respostas na reply.txt ou use o comando '/resposta', Peppy também consegue adicionar ela mesma sozinha
			case 'peppy':; case 'pp':
				...
				try {
					...
					await kill.reply(from, chatBotR, id)
					console.log(color('[PEPPY]', 'crimson'), color(`→ Obtive erros no comando ${prefix}${command} → ${error.message} - Você pode ignorar.`, 'gold'))
				}
				break
		   ...
		   case 'clima':
			...
			case 'report':
				if (args.length == 0) return await kill.reply(from, mess.noargs() + 'motivo.', id)
			...
			case 'ajudar':
				if (!isGroupMsg) return await kill.reply(from, mess.ajudar(), id)
				if (isGroupMsg) return await kill.reply(from, mess.ajudar(), id)
			break
